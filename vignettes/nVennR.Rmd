...
```{r "plotVenn", dev='svg', fig.show='hold', fig.keep='all'}
library(nVennR)
exampledf
...
```{r "Low-level", dev='svg', fig.show='hold', fig.keep='all'}
myV3 <- createVennObj(nSets = 5, sSizes = c(rep(1, 32)))
myV3 <- plotVenn(nVennObj = myV3)
myV3 <- plotVenn(nVennObj = myV3)
```

Incidentally, these figures show another reason to run `plotVenn` repeatedly rather than raising `nCycles`. Each time the function runs, the speed of the simulation is reset, which may make convergence much faster. If we had run `myV3 <- plotVenn(nVennObj = myV3, nCyles=14000)`, the resulting figure would have been very similar to the left panel.
...
```{r "opacity", dev='svg', fig.show='hold', fig.keep='all'}
...
```{r "setColors", dev='svg', fig.show='hold', fig.keep='all'}
...
```{r "showLabels", dev='svg', fig.show='hold', fig.keep='all'}
...
This function returns a list of all the regions in the diagram. In turn, each region contains a list of the elements in it. Empty regions can be listed by setting `na.rm` to `FALSE`.
